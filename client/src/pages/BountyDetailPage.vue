...
async function loadDetail() {
  ...
  try {
    ...
    errorMessage.value = error.message || "加载悬赏详情失败。";
  } finally {
    loading.value = false;
  }
}
...
async function applyToBounty() {
  ...
  try {
    ...
    errorMessage.value = error.message || "申请失败，请稍后重试。";
  } finally {
...
async function reviewApplication(item, decision) {
  ...
  try {
    ...
    errorMessage.value = error.message || "审核失败，请稍后重试。";
  } finally {
    reviewingId.value = 0;
  }
}
...
async function updateStatus() {
  ...
  try {
    ...
    errorMessage.value = error.message || "状态更新失败，请稍后重试。";
