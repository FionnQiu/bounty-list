...
import { useProfilePage } from "../composables/useProfilePage.js";
    ...
    <section v-else-if="profile" class="profile-shell">
      ...
      <section class="profile-sections">
          ...
          <div class="section-body">
            <dl class="account-list">
              <div class="account-row">
                ...
                <button class="copy-btn" type="button" :disabled="!profile.user.username" @click="copyField(profile.user.username, '用户名')">复制</button>
              </div>

              <div class="account-row">
                <dt>手机号</dt>
                <dd :title="profile.user.phone || '未设置'">{{ profile.user.phone || "未设置" }}</dd>
                <button class="copy-btn" type="button" :disabled="!profile.user.phone" @click="copyField(profile.user.phone, '手机号')">复制</button>
              </div>

              <div class="account-row">
                <dt>邮箱</dt>
                <dd :title="profile.user.email || '未设置'">{{ profile.user.email || "未设置" }}</dd>
                <button class="copy-btn" type="button" :disabled="!profile.user.email" @click="copyField(profile.user.email, '邮箱')">复制</button>
              </div>

              <div class="account-row account-row--bio">
                <dt>简介</dt>
                <dd :title="profile.user.bio || '暂无简介'">{{ profile.user.bio || "暂无简介" }}</dd>
                <button class="copy-btn" type="button" :disabled="!profile.user.bio" @click="copyField(profile.user.bio, '简介')">复制</button>
              </div>
            </dl>
          </div>
        </article>
        ...
        <article class="profile-card profile-section">
          ...
          <div class="section-body section-body--overview">
            ...
            <div class="overview-links">
              ...
              <button class="btn btn-outline" type="button" @click="goMessages">进入对话</button>
      ...
      <article class="profile-card ratings-panel">
        <header class="section-head ratings-head">
          ...
          <div class="ratings-summary">
            <span class="summary-chip">已发出 {{ ratingsSummary.sentCount }}</span>
            <span class="summary-chip">已获得 {{ ratingsSummary.receivedCount }}</span>
            <span class="summary-chip">平均分 {{ formatAverageScore(ratingsSummary.receivedAverageScore) }}</span>
          </div>
        </header>
        ...
        <div class="section-body ratings-body">
          ...
          <p v-else-if="ratingsErrorMessage" class="feedback feedback--error">{{ ratingsErrorMessage }}</p>
          ...
          <ul v-else class="rating-list" :aria-label="ratingTab === 'sent' ? '已发出的评价列表' : '已获得的评价列表'">
            <li v-for="item in activeRatings" :key="`${ratingTab}-${item.id}`" class="rating-item" tabindex="0">
              ...
              <p class="rating-item__comment" :title="item.comment || '未填写评语'">
                {{ item.comment || "未填写评语" }}
              </p>
          ...
          <div class="edit-avatar">
            ...
            <div class="edit-avatar__actions">
              <div class="edit-avatar__file-shell">
                <label class="edit-avatar__label" for="avatar-upload">上传新头像</label>
                <input id="avatar-upload" class="edit-avatar__input" type="file" accept="image/*" @change="onAvatarFileChange" />
              </div>
              <small class="edit-avatar__hint">支持常见图片格式，建议上传 2MB 以内图片。</small>
            </div>
          </div>
        </section>
...
<style scoped>
.profile-page {
  min-height: calc(100dvh - var(--top-dock-height));
  display: flex;
